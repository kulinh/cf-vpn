...
// assetArch maps the Go arch to the suffix cloudflared and lego use in their
// release file names (they happen to agree). Xray and Hysteria are installed
// through their official scripts, which detect the architecture themselves.
func assetArch() (string, error) {
	switch goarch {
	case "amd64", "arm64":
		return goarch, nil
	}
	return "", fmt.Errorf("no linux/%s release assets for cloudflared and lego (supported: amd64, arm64)", goarch)
}
