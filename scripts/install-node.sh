...
# AGENT_SHARED_SECRET gates /admin/v1/* on the publicly-resolvable admin
# tunnel; the agent now refuses to start when it is empty. Generate one if
# the operator did not provide it. The same value MUST be mirrored into the
# node's row in D1 (column: agent_shared_secret) so the Worker can authenticate.
...
# Pass CF_API_TOKEN via curl --config stdin so it never appears in argv
# (visible to other users via `ps` / /proc/<pid>/cmdline).
D1_RESP=$(curl -sS --max-time 10 --config - <<EOF
...
# Mirror AGENT_SHARED_SECRET into D1 nodes.agent_secret so the Worker
# sends the correct per-node bearer on /admin/v1/*. Non-fatal: the row
# may not exist yet (operator may add this node to D1 via the panel
# afterwards). Token passed via curl --config to keep it out of argv.
log "mirroring agent_secret into D1 (nodes.agent_secret WHERE id=$NODE_ID)"
D1_DB_ID="${CFVPN_D1_DATABASE_ID:-0649f07f-e2c0-47f3-b84a-273f7f67332e}"
SECRET_PAYLOAD="$(mktemp)"
chmod 600 "$SECRET_PAYLOAD"
jq -n --arg s "$AGENT_SHARED_SECRET" --arg id "$NODE_ID" \
  '{sql:"UPDATE nodes SET agent_secret=? WHERE id=?", params:[$s,$id]}' > "$SECRET_PAYLOAD"
SECRET_RESP=$(curl -sS --max-time 30 --config - <<EOF
header = "Authorization: Bearer ${CF_API_TOKEN}"
...
rm -f "$SECRET_PAYLOAD"
SECRET_OK=$(echo "$SECRET_RESP" | jq -r '.success // false' 2>/dev/null || echo false)
...
# ----- 7. run installer -------------------------------------------------------
# Snapshot existing cfvpn-admin-* tunnels so we can detect orphans created
# by a partial install (tunnel created in CF, but install failed before
# persisting the UUID into cfvpn.env — we can't recover it from local state).
list_admin_tunnels() {
