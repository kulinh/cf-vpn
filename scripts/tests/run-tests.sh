...
# Emulates OpenSSH exactly: argv[1..] after the host are joined with SPACES
# into one string that the remote LOGIN SHELL re-parses. Local quoting is gone.
fake_ssh() {
...
# --- new form: ssh_run bash -s <<'REMOTE' + printf %q re-quoting -------------
new_ssh_run() { fake_ssh -o BatchMode=yes root@target "$(printf '%q ' "$@")"; }
new_out="$(CFVPN_ENV_FILE="$REMOTE_ENV_FILE" new_ssh_run env "CFVPN_ENV_FILE=$REMOTE_ENV_FILE" bash -s <<'REMOTE'
set -euo pipefail
...
# FORCE_REINSTALL=1: backup first, then a clean file
...
is "$(grep -c '^ADMIN_TUNNEL_UUID=tunnel-keep$' "$ENVF")" "1" "preserves the admin tunnel id"
...
mkdir -p "$(dirname "$ENVF2")"
{
  printf 'CF_API_TOKEN=%s\n' "tok-en_123"
  printf 'NODE_ID=%s\n'      "chn-02"
} | fake_ssh -o BatchMode=yes root@target \
      "$(printf '%q ' env "CFVPN_ENV_FILE=$ENVF2" FORCE_REINSTALL=0 bash "$LIB/cfvpn-env-file.sh" write)" >/dev/null
